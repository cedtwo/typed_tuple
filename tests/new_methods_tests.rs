...
fn test_replace_middle() {
    let mut tuple = ("hello", 42, 2.5);
    let old = TypedTuple::<TupleIndex1, _>::replace(&mut tuple, 99);
    assert_eq!(old, 42);
...
fn test_replace_last() {
    ...
    let old = TypedTuple::<TupleIndex2, _>::replace(&mut tuple, 100u32);
    assert_eq!(old, 3u32);
...
fn test_swap_no_op_same_index() {
    let mut tuple = (1u8, 2u16, 3u32);
    TypedTuple::<TupleIndex1, u16>::swap::<TupleIndex1>(&mut tuple);
...
fn test_swap_strings() {
    ...
    TypedTuple::<TupleIndex0, &str>::swap::<TupleIndex2>(&mut tuple);
    assert_eq!(tuple, ("b", 42, "a", 2.5, "c"));

    TypedTuple::<TupleIndex2, &str>::swap::<TupleIndex4>(&mut tuple);
...
fn test_take_vec() {
    ...
    let v = TypedTuple::<TupleIndex1, Vec<i32>>::take(&mut tuple);
...
fn test_take_number() {
    let mut tuple = (1u8, 2u16, 3u32);
    let val = TypedTuple::<TupleIndex2, u32>::take(&mut tuple);
    assert_eq!(val, 3u32);
...
fn test_combined_operations() {
    ...
    TypedTuple::<TupleIndex0, u8>::swap::<TupleIndex3>(&mut tuple);
    assert_eq!(tuple, (4u8, 2u16, 3u32, 1u8, 5u16));

    // Replace an element
    let old = TypedTuple::<TupleIndex2, u32>::replace(&mut tuple, 99u32);
    assert_eq!(old, 3u32);
