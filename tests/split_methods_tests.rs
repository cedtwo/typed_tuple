...
fn test_split_exclusive_first() {
    ...
    let (left, element, right) = tuple.split_exclusive::<TupleIndex0>();
    assert_eq!(left, ());
...
fn test_all_split_methods_together() {
    ...
    let (left_ex, elem, right_ex) = tuple1.split_exclusive::<TupleIndex2>();
...
fn test_split_single_element() {
    ...
    let (left, elem, right) = tuple.split_exclusive::<TupleIndex0>();
    assert_eq!(left, ());
