...
use typed_tuple::prelude::{TypedIndex, TypedTuple};

// Test custom trait with blanket implementation using TypedIndex
...
fn test_element_at_trait() {
    ...
    assert_eq!(*ElementAt::<typenum::U0, u32>::get_at(&tuple_a), 42u32);
    let old = ElementAt::<typenum::U0, u32>::replace_at(&mut tuple_a, 100u32);
    assert_eq!(old, 42u32);
    assert_eq!(tuple_a, (100u32,));

    let mut tuple_b = (10u32, "hello", 3.5);
    assert_eq!(*ElementAt::<typenum::U0, u32>::get_at(&tuple_b), 10u32);
    let old = ElementAt::<typenum::U0, u32>::replace_at(&mut tuple_b, 99u32);
    assert_eq!(old, 10u32);
    assert_eq!(tuple_b, (99u32, "hello", 3.5));

    // Access element at index 1
    assert_eq!(*ElementAt::<typenum::U1, &str>::get_at(&tuple_b), "hello");
    ElementAt::<typenum::U1, &str>::replace_at(&mut tuple_b, "changed");
    assert_eq!(tuple_b, (99u32, "changed", 3.5));

    // Access element at index 2
    assert_eq!(*ElementAt::<typenum::U2, f64>::get_at(&tuple_b), 3.5);
    ElementAt::<typenum::U2, f64>::replace_at(&mut tuple_b, 2.71);
